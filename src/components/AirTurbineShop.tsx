...
export default function AirTurbineShop({ whatsappNumber = "919999999999" }: { whatsappNumber?: string }) {
  const openWhatsApp = (message = "Hello, I need help with turbines and ventilators.") => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 text-gray-900 antialiased">
        ...
        >
          <div>
            ...
            <p className="mt-4 text-lg text-slate-600">
              Silent. Efficient. Durable. Delivering optimized airflow solutions for homes, offices and heavy industry.
            </p>
            ...
            <div className="mt-6 flex gap-3">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => openWhatsApp("Hello, I want a quote for bulk industrial turbines.")}
          ...
          >
            <div className="w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
              <motion.div className="relative w-full h-72">
                <Image
              ...
              <div className="p-4 bg-white">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-semibold">{SAMPLE_PRODUCTS[0].name}</div>
                    <div className="text-sm text-slate-500">{SAMPLE_PRODUCTS[0].description}</div>
                  </div>
                  <div className="text-lg font-bold">{formatINR(SAMPLE_PRODUCTS[0].price)}</div>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <main className="container mx-auto px-6 pb-24">
            ...
            {SAMPLE_PRODUCTS.map((p) => (
              ...
              >
                <div className="relative h-56">
                  <Image src={p.img} alt={p.name} className="w-full h-full object-cover" width={500} height={500} />
                  <div className="absolute top-3 left-3 bg-white/80 text-xs px-3 py-1 rounded-full">{p.tags.join(" • ")}</div>
                </div>
                <div className="p-4">
                  ...
                  <div className="mt-4 flex gap-3">
                    ...
                    <button
                      onClick={() => openWhatsApp(`Hi, I want a quote for ${p.name} (${p.id}). Quantity:`)}
        ...
        <section className="mt-12 grid md:grid-cols-2 gap-8 items-start">
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            ...
            <p className="mt-2 text-sm text-slate-500">Share details and we'll respond on WhatsApp or email within 6 business hours.</p>
            <div className="mt-4 grid grid-cols-1 gap-3">
              <input type="text" placeholder="Company / Name" className="border px-3 py-2 rounded" />
              <input type="tel" placeholder="Phone (so we can WhatsApp)" className="border px-3 py-2 rounded" />
              <textarea placeholder="Project details & quantity" className="border px-3 py-2 rounded h-24"></textarea>
              <div className="flex gap-3">
                <button
                  onClick={() => openWhatsApp("Hi, I need a tailored quote. Here are my details: ")}
      ...
      <div className="fixed right-6 bottom-6 z-50">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={() => openWhatsApp("Hello — I need product & installation support.")}
      ...
      <footer className="mt-24 bg-slate-900 text-white">
        <div className="container mx-auto px-6 py-10 grid md:grid-cols-3 gap-6">
          <div>
            <div className="text-xl font-bold">Zunera air ventilators & turbines Co.</div>
            <div className="mt-3 text-sm text-slate-300">Design, manufacture & service of industrial and domestic ventilation systems.</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="mt-3 text-sm text-slate-300">Phone: +91 9019673295 /8445974216</div>
            <div className="text-sm text-slate-300">Email: zunera.airventilators.gmail.com</div>
          </div>
        </div>
        <div className="border-t border-slate-800 text-slate-500 text-sm py-4 text-center">
          © {new Date().getFullYear()} Zunera Co. All rights reserved. developed by Abdul sattar shaikh
        </div>
      </footer>
    </div>
  );
